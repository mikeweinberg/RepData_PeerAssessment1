...
Mean total steps is `r mean_total_steps` and median total steps is `r median_total_steps`
...
The 5-minute interval that, on average, contains the maximum number of steps is `r max_interval` with `r max_steps` steps.

## Imputing missing values
#### Note that there are a number of days/intervals where there are missing values (coded as NA). The presence of missing days may introduce bias into some calculations or summaries of the data.

1. The total number of missing values in the dataset (i.e. the total number of rows with NAs) is **`r sum(is.na(activity$steps))`** rows out of **`r length(activity$steps)`** rows total.
...
Mean total steps for the new data is `r new_mean_total_steps` and median total steps is `r new_median_total_steps`. There wasn't a change from previously when `r mean_total_steps` was mean total steps and  `r median_total_steps` was median total steps.
